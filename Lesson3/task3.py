    ...
    while nextpage and npage<maxpage:
        ...
        if response.ok:
            ...
            for serial in serials_list:
                ...
                serial_data['key']=serial_data['link']
                serial_data['name'] = serial.find('a',{'class':'bloko-link HH-LinkModifier'}).string
                key=re.search("vacancy\/(\d+)", serial_data['link'])
                if key:
                    serial_data['key'] = 'hh'+ key[1]
        ...
        if response.ok:
            ...
            if serials_list:
                for serial in serials_list:
                    ...
                    key=re.search("vakansii\/\D+\-(\d+)\.html", serial_data['link'])
                    if key:
                        serial_data['key'] = 'sj'+ key[1]
...
for vac in vacancies:
    countvac = session.query(Vacancy).filter(Vacancy.key == vac['key']).count()
    if countvac == 0:
        session.add(Vacancy(vac['key'],vac['name'],vac['link'],vac['host'],vac['compensation'],vac['minsalary'],vac['maxsalary'],vac['currency']))
        print ('Добавляем вакансию ',vac['key'])
        session.commit()
